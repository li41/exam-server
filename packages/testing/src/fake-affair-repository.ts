...
class InMemoryAffairRepository implements AffairRepository {
  ...
  async updateAffair(
    ...
    const index = this.affairs.findIndex(
      (item) => item.id === id && item.tenantId === scope.tenantId,
    );
  ...
  async initializeCities(
    ...
    for (const [code, name] of cities) {
      if (
        this.cityRows.some(
          (item) => item.tenantId === scope.tenantId && item.cityCode === code,
        )
      )
        continue;
      const timestamp = now();
  ...
  async updateCity(
    ...
    const index = this.cityRows.findIndex(
      (item) => item.id === id && item.tenantId === scope.tenantId,
    );
    if (index < 0) throw new NotFoundError("affair city", id);
    const current = this.cityRows[index] as AffairCity;
    if (current.version !== input.version)
      throw new ConflictError("affair city was modified by another request.");
    const { version: _version, ...changes } = input;
    const updated = {
      ...current,
      ...changes,
  ...
  async listSchools(
    ...
    if (!(await this.getAffair(query.affairId, scope)))
      throw new NotFoundError("affair", query.affairId);
    const search = query.search?.toLocaleLowerCase();
    return page(
      this.schools.filter(
        (item) =>
          ...
          (!query.schoolLevel || item.schoolLevel === query.schoolLevel) &&
          (!query.status || item.status === query.status) &&
          (!search ||
            item.schoolCode.toLocaleLowerCase().includes(search) ||
            item.schoolName.toLocaleLowerCase().includes(search)),
      ),
      query.limit,
    );
  }
  ...
  async createSchool(
    ...
    if (!(await this.getAffair(input.affairId, scope)))
      throw new NotFoundError("affair", input.affairId);
    if (
      this.schools.some(
        (item) =>
          item.tenantId === scope.tenantId &&
          item.affairId === input.affairId &&
          item.schoolLevel === input.schoolLevel &&
    ...
    const item: AffairSchool = {
      ...
      testClasses: input.testClasses,
      testSessions: input.testSessions,
      receiptCode: input.receiptCode,
      briefingOptions: input.briefingOptions,
      password: input.password || input.schoolCode,
      contacts: null,
      setupCompleted: null,
      status: input.status,
      version: 1,
  ...
  async updateSchool(
    ...
    if (current.version !== input.version)
      throw new ConflictError("affair school was modified by another request.");
  ...
  async deleteSchool(
    ...
    if (index < 0) throw new NotFoundError("affair school", id);
    if ((this.schools[index] as AffairSchool).version !== version)
      throw new ConflictError("affair school was modified by another request.");
    this.schools.splice(index, 1);
  }
}

export const createInMemoryAffairRepository = (): AffairRepository =>
  new InMemoryAffairRepository();
