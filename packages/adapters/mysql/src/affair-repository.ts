...
type AffairRow = RowDataPacket & {
  ...
  city_login_start: Date | string | null;
  city_login_end: Date | string | null;
  school_login_start: Date | string | null;
  school_login_end: Date | string | null;
  fee_city_contact: number;
  fee_school_contact: number;
  fee_teacher_setup: number;
  fee_teacher_monitor_1: number;
  fee_teacher_monitor_2: number;
  fee_teacher_monitor_3: number;
  transport_receipt_school: number;
  transport_receipt_city: number;
  briefing_regions: unknown;
  receipt_year: string | null;
  receipt_note: string | null;
  receipt_print_school: number;
  receipt_print_city: number;
  version: number;
...
type SchoolRow = RowDataPacket & {
  ...
  briefing_options: unknown;
  password: string;
  contacts: unknown;
  setup_completed: unknown;
  status: AffairSchool["status"];
  version: number;
  created_at: Date | string;
  updated_at: Date | string;
};
...
const affairColumns = `
  a.id, a.tenant_id, a.created_by, a.name, a.description, a.status,
...
const schoolColumns = `
  s.id, s.affair_id, s.tenant_id, s.city, s.school_level,
...
const toIso = (value: Date | string | null): string | null => {
  ...
  if (Number.isNaN(date.getTime())) {
    throw new Error("MySQL returned an invalid affair date.");
  }
...
const parseJson = (value: unknown): unknown => {
  ...
  try {
    ...
    throw new Error("MySQL returned invalid affair JSON.");
  }
};
...
export class MySqlAffairRepository implements AffairRepository {
  ...
  async listAffairs(
    ...
    if (query.search) {
      predicates.push("a.name LIKE ? ESCAPE '!'");
    ...
    return {
      items: visible.map((row) => this.toAffair(row)),
      page: {
        nextCursor:
          hasNext && last
            ? encodeItemCursor({
  ...
  async createAffair(
    ...
    await this.pool.execute(
      ...
      [
        ...
        input.description,
        input.status,
        input.cityLoginStart ? new Date(input.cityLoginStart) : null,
        input.cityLoginEnd ? new Date(input.cityLoginEnd) : null,
        input.schoolLoginStart ? new Date(input.schoolLoginStart) : null,
        input.schoolLoginEnd ? new Date(input.schoolLoginEnd) : null,
        input.feeCityContact,
        input.feeSchoolContact,
        input.feeTeacherSetup,
        input.feeTeacherMonitor1,
        input.feeTeacherMonitor2,
        input.feeTeacherMonitor3,
        input.transportReceiptSchool ? 1 : 0,
        input.transportReceiptCity ? 1 : 0,
        encodeJson(input.briefingRegions),
        input.receiptYear,
        input.receiptNote,
        input.receiptPrintSchool ? 1 : 0,
        input.receiptPrintCity ? 1 : 0,
        now,
        now,
      ],
    );
    ...
    if (!affair) {
      throw new Error("Affair insert succeeded but could not be read.");
    }
    return affair;
  }
  ...
  async updateAffair(
    ...
    if (input.cityLoginStart !== undefined) {
      put(
        "city_login_start",
        input.cityLoginStart ? new Date(input.cityLoginStart) : null,
      );
    }
    if (input.cityLoginEnd !== undefined) {
      put(
        "city_login_end",
        input.cityLoginEnd ? new Date(input.cityLoginEnd) : null,
      );
    }
    if (input.schoolLoginStart !== undefined) {
      put(
        "school_login_start",
        input.schoolLoginStart ? new Date(input.schoolLoginStart) : null,
      );
    }
    if (input.schoolLoginEnd !== undefined) {
      put(
        "school_login_end",
        input.schoolLoginEnd ? new Date(input.schoolLoginEnd) : null,
      );
    ...
    const feeColumns = [
      ["feeCityContact", "fee_city_contact"],
      ["feeSchoolContact", "fee_school_contact"],
      ["feeTeacherSetup", "fee_teacher_setup"],
      ["feeTeacherMonitor1", "fee_teacher_monitor_1"],
      ["feeTeacherMonitor2", "fee_teacher_monitor_2"],
      ["feeTeacherMonitor3", "fee_teacher_monitor_3"],
    ] as const;
    for (const [key, column] of feeColumns) {
      if (input[key] !== undefined) put(column, input[key]);
    }
    if (input.transportReceiptSchool !== undefined) {
      put("transport_receipt_school", input.transportReceiptSchool ? 1 : 0);
    }
    if (input.transportReceiptCity !== undefined) {
      put("transport_receipt_city", input.transportReceiptCity ? 1 : 0);
    }
    if (input.briefingRegions !== undefined) {
      put("briefing_regions", encodeJson(input.briefingRegions));
    }
    ...
    if (input.receiptPrintSchool !== undefined) {
      put("receipt_print_school", input.receiptPrintSchool ? 1 : 0);
    }
    if (input.receiptPrintCity !== undefined) {
      put("receipt_print_city", input.receiptPrintCity ? 1 : 0);
    }
    ...
    for (const [code, name] of cityList) {
      ...
      const [result] = await this.pool.execute<ResultSetHeader>(
        `INSERT IGNORE INTO affair_cities (
          id, tenant_id, city_code, city_name, account, password,
          setup_completed, version, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, 0, 1, ?, ?)`,
        [
          randomUUID(),
          scope.tenantId,
  ...
  async listSchools(
    ...
    if (query.city) {
      predicates.push("s.city = ?");
      params.push(query.city);
    }
    if (query.schoolLevel) {
      predicates.push("s.school_level = ?");
      params.push(query.schoolLevel);
    }
    if (query.status) {
    ...
    if (query.search) {
      predicates.push(
        "(s.school_code LIKE ? ESCAPE '!' OR s.school_name LIKE ? ESCAPE '!')",
      );
    ...
    const last = visible.at(-1);
    return {
      items: visible.map((row) => this.toSchool(row)),
      page: {
        nextCursor:
          hasNext && last
            ? encodeItemCursor({
  ...
  async createSchool(
    ...
    try {
      await this.pool.execute(
        ...
        [
          ...
          input.testSessions,
          input.receiptCode,
          encodeJson(input.briefingOptions),
          input.password || input.schoolCode,
          input.status,
          now,
          now,
        ],
      ...
      if (isDuplicate(error)) {
        throw new ConflictError(
          "A school with the same code and level already exists in this affair.",
        );
      }
      throw error;
    }
    ...
    if (!school) {
      throw new Error("Affair school insert succeeded but could not be read.");
    }
    return school;
  }
  ...
  private async throwVersionFailure(
    table: "affairs" | "affair_cities" | "affair_schools",
    resource: string,
    id: string,
    version: number,
    scope: QuestionBankScope,
    ...
    const [rows] = await this.pool.execute<VersionRow[]>(
      `SELECT version FROM ${table} WHERE id = ? AND tenant_id = ? LIMIT 1`,
      [id, scope.tenantId],
    );
  ...
  private toAffair(row: AffairRow): Affair {
    return {
      ...
      status: row.status,
      cityLoginStart: toIso(row.city_login_start),
      cityLoginEnd: toIso(row.city_login_end),
      schoolLoginStart: toIso(row.school_login_start),
      schoolLoginEnd: toIso(row.school_login_end),
      feeCityContact: Number(row.fee_city_contact),
      feeSchoolContact: Number(row.fee_school_contact),
      feeTeacherSetup: Number(row.fee_teacher_setup),
      feeTeacherMonitor1: Number(row.fee_teacher_monitor_1),
      feeTeacherMonitor2: Number(row.fee_teacher_monitor_2),
      feeTeacherMonitor3: Number(row.fee_teacher_monitor_3),
      transportReceiptSchool: Boolean(row.transport_receipt_school),
      transportReceiptCity: Boolean(row.transport_receipt_city),
      briefingRegions: parseJson(row.briefing_regions),
      receiptYear: row.receipt_year,
      receiptNote: row.receipt_note,
      receiptPrintSchool: Boolean(row.receipt_print_school),
      receiptPrintCity: Boolean(row.receipt_print_city),
  ...
  private toSchool(row: SchoolRow): AffairSchool {
    const briefing = parseJson(row.briefing_options);
    const setup = parseJson(row.setup_completed);
    return {
      id: row.id,
      tenantId: row.tenant_id,
      affairId: row.affair_id,
      city: row.city,
      schoolLevel: Number(row.school_level) as 1 | 2 | 3,
      schoolCode: row.school_code,
      schoolName: row.school_name,
      testClasses: Number(row.test_classes) as 1 | 2,
      testSessions: Number(row.test_sessions) as 1 | 2 | 3,
      receiptCode: row.receipt_code,
      briefingOptions: Array.isArray(briefing) ? briefing.map(String) : null,
      password: row.password,
      contacts: parseJson(row.contacts),
      setupCompleted: Array.isArray(setup)
        ? setup.filter(
            (value): value is "SC" | "SD" | "SE" =>
              value === "SC" || value === "SD" || value === "SE",
          )
        : null,
      status: row.status,
      version: Number(row.version),
