...
import {
  API_VERSION,
  API_VERSION_PREFIX,
  AffairCollectionListQuerySchema,
  CreateAffairCollectionSchema,
  CreateAffairExcelFieldSchema,
  DeleteAffairExcelFieldQuerySchema,
  LEGACY_API_PREFIX,
  ReplaceAffairCollectionBindingsSchema,
  ReplaceAffairReferenceDataSchema,
  UpdateAffairCollectionSchema,
  UpdateAffairExcelFieldSchema,
...
type MountTarget = {
  route(path: string, app: Hono<AffairConfigurationEnv>): unknown;
};
...
const bearerToken = (context: Context<AffairConfigurationEnv>): string => {
...
const idempotencyKeyFor = (
  context: Context<AffairConfigurationEnv>,
): string | undefined => {
...
const createAffairConfigurationRouter = (dependencies: Dependencies) => {
  const api = new Hono<AffairConfigurationEnv>();
  const service = new AffairConfigurationService(dependencies.repository);
  ...
  api.post(
    "/affair-collections",
    zValidator("json", CreateAffairCollectionSchema, (result, context) => {
      if (!result.success) {
        return validationError(context, "Invalid affair collection payload.");
      }
    }),
    async (context) =>
      context.json(
  ...
  api.patch(
    "/affair-collections/:id",
    zValidator("json", UpdateAffairCollectionSchema, (result, context) => {
      if (!result.success) {
        return validationError(context, "Invalid affair collection payload.");
      }
    }),
  ...
  api.get("/affair-collections/:id/bindings", async (context) =>
    context.json(
      await service.listBindings(context.req.param("id"), scopeFor(context)),
    ),
  );

  api.put(
    "/affair-collections/:id/bindings",
    zValidator(
      "json",
      ReplaceAffairCollectionBindingsSchema,
      (result, context) => {
        if (!result.success) {
          return validationError(context, "Invalid affair binding payload.");
        }
      },
    ),
    async (context) =>
  ...
  api.get("/affair-collections/:id/reference-data", async (context) =>
    context.json(
      await service.listReferenceData(
        context.req.param("id"),
        scopeFor(context),
      ),
    ),
  ...
  api.get("/affair-fields/:id", async (context) =>
    context.json(
      await service.getField(context.req.param("id"), scopeFor(context)),
    ),
  );

  api.post(
    "/affair-fields",
    zValidator("json", CreateAffairExcelFieldSchema, (result, context) => {
      if (!result.success) {
        return validationError(context, "Invalid affair field payload.");
      }
    }),
    async (context) =>
      context.json(
  ...
  api.patch(
    "/affair-fields/:id",
    zValidator("json", UpdateAffairExcelFieldSchema, (result, context) => {
      if (!result.success) {
        return validationError(context, "Invalid affair field payload.");
      }
    }),
...
export const mountAffairConfigurationRoutes = (
  app: MountTarget,
  dependencies: Dependencies,
): void => {
  app.route(LEGACY_API_PREFIX, createAffairConfigurationRouter(dependencies));
  app.route(API_VERSION_PREFIX, createAffairConfigurationRouter(dependencies));
};
