...
const createAffairRouter = (dependencies: Dependencies) => {
  ...
  api.get(
    ...
    zValidator("query", AffairListQuerySchema, (result, context) => {
      if (!result.success)
        return validationError(context, "Invalid affair query parameters.");
    }),
    async (context) =>
      context.json(
        await service.listAffairs(
          context.req.valid("query"),
          scopeFor(context),
        ),
      ),
  );
  api.get("/affairs/:id", async (context) =>
    context.json(
      await service.getAffair(context.req.param("id"), scopeFor(context)),
    ),
  );
  api.post(
    "/affairs",
    zValidator("json", CreateAffairSchema, (result, context) => {
      if (!result.success)
        return validationError(context, "Invalid affair payload.");
    }),
    async (context) =>
      context.json(
        await service.createAffair(
  ...
  api.get("/affair-cities", async (context) =>
    context.json(await service.listCities(scopeFor(context))),
  );
  api.post("/affair-cities/initialize", async (context) =>
    context.json(await service.initializeCities(scopeFor(context)), 201),
  );
  api.patch(
    "/affair-cities/:id",
    zValidator("json", UpdateAffairCitySchema, (result, context) => {
      if (!result.success)
        return validationError(context, "Invalid affair city payload.");
    }),
    async (context) =>
      context.json(
    ...
    zValidator("query", AffairSchoolListQuerySchema, (result, context) => {
      if (!result.success)
        return validationError(
          context,
          "Invalid affair school query parameters.",
        );
    }),
    async (context) =>
      context.json(
  ...
  api.get("/affair-schools/:id", async (context) =>
    context.json(
      await service.getSchool(context.req.param("id"), scopeFor(context)),
    ),
  );
  api.post(
    "/affair-schools",
    zValidator("json", CreateAffairSchoolSchema, (result, context) => {
      if (!result.success)
        return validationError(context, "Invalid affair school payload.");
    }),
    async (context) =>
      context.json(
...
export const mountAffairRoutes = (
  app: MountTarget,
